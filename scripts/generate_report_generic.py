...
"""
import pickle, html, sys
import numpy as np

BUNDLE = sys.argv[1] if len(sys.argv) > 1 else "/tmp/final_sim_bundle.pkl"
OUT    = sys.argv[2] if len(sys.argv) > 2 else "/home/user/champion-trader/docs/rs_ema50_final_report.html"
...
nc   = B["nifty_curve"]       # (date, close) — benchmark
...
BENCH    = cfg.get("benchmark", "Nifty 50")
IS_USD   = (CUR == "USD")
MARKET   = "S&P 500" if IS_USD else "NSE"
TAX_NAME = "US Capital Gains" if IS_USD else "Indian Capital Gains"
ADT_LBL  = (f"${cfg['min_adt_cr']:.0f}M" if IS_USD else f"₹{cfg['min_adt_cr']:.0f}cr")
TAX_NOTE = (f"Per calendar tax year on realised gains. <b>STCG {cfg['stcg_rate']:.0f}%</b> "
            f"(short-term, &lt; 12 months, taxed as ordinary income) and <b>LTCG {cfg['ltcg_rate']:.0f}%</b> "
            f"(long-term, ≥ 12 months). Assumes a representative investor bracket — adjust to taxpayer's actual rates. "
            f"Open positions at period end are untaxed."
            if IS_USD else
            f"Per financial year (Apr–Mar) on realised gains. <b>STCG {cfg['stcg_rate']:.0f}%</b> for holdings &lt; 12 months; "
            f"<b>LTCG {cfg['ltcg_rate']:.1f}%</b> for ≥ 12 months above the annual <b>{{exempt}}</b> exemption "
            f"(post-July-2024 rates). Open positions at period end are untaxed.")
REGIME_NOTE = ("2021–2025 was broadly bullish for US large-caps (Mag-7 led)" if IS_USD
               else "2021–2025 was broadly bullish for Indian equities")

# ─── number formatting (currency-aware) ──────────────────────────────────────
def inr(x, dec=0):
    neg = x < 0; x = abs(x)
    if IS_USD:
        return ("-" if neg else "") + "$" + f"{x:,.{dec}f}"
    s = f"{x:.{dec}f}"
...
def lakh(x):
    """Headline 'big number' formatter."""
    if IS_USD: return "$" + f"{x:,.0f}"
    return f"₹{x/1e5:.2f}L"

CSYM = "$" if IS_USD else "₹"
TAX_NOTE_R = TAX_NOTE if IS_USD else TAX_NOTE.replace("{exempt}", inr(cfg['ltcg_exempt']))
...
chart_equity=svg_dual_line(eq_series,bench_series,
    ["RS EMA50×200 Strategy",f"{BENCH} (buy & hold)"],["#3fb950","#4a7dbf"],
    y_is_currency=True, hline=CAP, markers=eq_markers)
chart_equity_log=svg_dual_line(eq_series,bench_series,
    ["RS EMA50×200 Strategy (log)",f"{BENCH} (log)"],["#3fb950","#4a7dbf"],
    y_is_currency=True, logscale=True)
chart_roll=svg_dual_line(roll_s,roll_n,
    ["Strategy rolling 12-mo return",f"{BENCH} rolling 12-mo return"],["#3fb950","#4a7dbf"],
...
def trade_rows(ts):
    ...
    for t in ts:
        ...
        r+=f'<tr><td class="sym">{html.escape(t["sym"])}</td><td>{t["entry_date"]}</td><td>{t["exit_date"]}</td><td class="num">{t["days"]}</td><td class="num">{CSYM}{t["entry"]:.1f}</td><td class="num">{CSYM}{t["exit"]:.1f}</td><td class="num {cls}">{t["pnl_pct"]:+.1f}%</td><td class="num {cls}">{inr(t["pnl"])}</td><td><span class="badge {rcls}">{t["reason"]}</span></td></tr>'
    return r
...
<header><div class="wrap">
  ...
  <div class="subtitle">Swing trading backtest on {MARKET} equities · {cfg["sim_start"]} → {cfg["sim_end"]} · {S["years"]:.1f} years</div>
  <div class="tagline">
    ...
    <span class="pill">Universe <b>{cfg["universe"]} stocks</b> (ADT ≥ {ADT_LBL})</span>
  </div>
</div></header>
...
<div class="note green"><b>Headline:</b> {lakh(CAP)} grew to <b>{lakh(S['final_value'])}</b>
({S['total_ret']:+.0f}% pre-tax / {S['post_tax_ret']:+.0f}% post-tax) over {S['years']:.1f} years —
a <b>{S['cagr']:.1f}% CAGR</b> versus {BENCH}'s {S['nifty_cagr']:.1f}%. The strategy delivered
...
<div class="lead">{lakh(CAP)} invested in the strategy vs the same {lakh(CAP)} in {BENCH}. Dashed gold line = starting capital floor; dots mark the peak and trough of the worst drawdown.</div>
...
<div class="grid grid4">
  ...
  {metric_card("Trough vs capital", f"+{trough_gain:.0f}%", f"never near original {lakh(CAP)}")}
  {metric_card("Drawdown length", f"{dd_days} days", (f"recovered in {recov_days}d" if recov_days else "recovering"))}
</div>
<div class="note">At the {dates[dd_peak_i]} peak, all {cfg['max_pos']} positions were large unrealised winners (e.g. {S['largest_win_sym']} {S['largest_win_pct']:+.0f}%). Their hard stops were ~10% below the <i>original entry</i>, i.e. far below market — so the
...
<h2>14 · Tax Analysis ({TAX_NAME})</h2>
<div class="note">{TAX_NOTE_R}</div>
<table>
...
<table>
  ...
  <tr><td class="sym">Universe filter</td><td>Mean daily turnover ≥ {ADT_LBL} across the sim period ({cfg['universe']} stocks qualify)</td></tr>
...
current {MARKET} liquid names; delisted/merged stocks are excluded. (4) Returns are concentrated in a few multi-baggers
(top 10 trades = {top10_share:.0f}% of gross profit). (5) {REGIME_NOTE} — forward
